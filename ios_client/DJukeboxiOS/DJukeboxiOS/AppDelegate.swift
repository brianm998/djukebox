...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    // Theme the navigation bars for the dark neon look: a dark translucent
    // background (so the screen gradient shows through) with bright titles and a
    // cyan tint for bar buttons.
    //
    // NOTE: we deliberately do NOT customize UITabBar.appearance() here. On
    // iPadOS 26 setting the tab bar's standard/scrollEdge appearance makes the
    // system render a *second* tab bar at the top of the window. The selected-tab
    // color already comes from the root `.tint(DJTheme.accent)`, so the tab bar
    // stays correctly themed without an appearance override.
