...
// Registers the routes that require auth (loopback or a paired token) onto
// `protected` (an `app.grouped(AuthMiddleware(...))`), except for
// `/stream/:auth/:sha1`, whose token travels as the `:auth` path parameter
// rather than the `Authorization` header. AuthMiddleware checks both, so that
// route is ALSO registered on `protected` — it just presents its credential
// differently. See AuthController.swift for the middleware itself.
func trackServingRoutes(_ protected: any RoutesBuilder) throws {

    // Json list of all known tracks
    // curl -H Authorization:<token> localhost:8080/tracks
    ...
    protected.get("tracks") { req -> [AudioTrack] in
        var ret: [AudioTrack] = []
    ...
    protected.get("stream", ":auth", ":sha1") { req async throws -> Response in
    ...
    // curl -H Authorization:<token> localhost:8080/info/8ba165d9fe8f1050687dfa0f34ab42df6a29e72c
    protected.get("info", ":sha1") { req -> AudioTrack in
        let authControl = AuthController(pairing: pairingService, trackFinder: trackFinder)
    ...
    // gated by AuthMiddleware ahead of this handler; the actual ingest is
    // offloaded so the event-loop thread is free while it runs.
    protected.get("discover") { req async throws -> Response in
        var path: String?
...
func historyRoutes(_ protected: any RoutesBuilder) throws {
    ...
    protected.get("history") { req -> PlayingHistory in
        return history.all
    }

    // json content of played tracks
    protected.get("history",  ":since") { req -> PlayingHistory in
    ...
    protected.post("history") { req async throws -> Response in
...
func playerRoutes(_ protected: any RoutesBuilder) throws {

    // Play a track by hash.
    // curl -H Authorization:<token> localhost:8080/play/8ba165d9fe8f1050687dfa0f34ab42df6a29e72c
    protected.get("play", ":sha1") { req -> AudioTrack in
        let authControl = AuthController(pairing: pairingService, trackFinder: trackFinder)
        return try authControl.track(from: req) { track, _ in
    ...
    // curl -H Authorization:<token> localhost:8080/rand
    protected.get("rand") { req -> AudioTrack in
    ...
    // curl -H Authorization:<token> localhost:8080/rand/Queen
    protected.get("rand", ":credit") { req -> AudioTrack in
    ...
    // curl -H Authorization:<token> localhost:8080/newrand
    protected.get("newrand") { req -> AudioTrack in
    ...
    // Play a randomly selected track by a given credit
    // curl -H Authorization:<token> localhost:8080/newrand/Queen
    protected.get("newrand", ":credit") { req -> AudioTrack in
        if let credit = req.parameters.get("credit") {
            let array = trackFinder.tracks(forCredit: credit)
    ...
    // curl -H Authorization:<token> localhost:8080/stop
    protected.get("stop") { req -> Response in
    ...
    // curl -H Authorization:<token> localhost:8080/stop/8ba165d9fe8f1050687dfa0f34ab42df6a29e72c
    protected.get("stop", ":sha1") { req -> Response in
        let authControl = AuthController(pairing: pairingService, trackFinder: trackFinder)
        return try authControl.track(from: req) { track, _ in
    ...
    // curl -H Authorization:<token> localhost:8080/stop/8ba165d9fe8f1050687dfa0f34ab42df6a29e72c/3
    protected.get("stop", ":sha1", ":index") { req -> Response in
        Log.d("stop at index")
        let authControl = AuthController(pairing: pairingService, trackFinder: trackFinder)
        return try authControl.track(from: req) { track, _ in
    ...
    protected.get("move", ":sha1", ":start", ":destination") { req -> PlayingQueue in
        let authControl = AuthController(pairing: pairingService, trackFinder: trackFinder)
        return try authControl.track(from: req) { track, _ in
    ...
    // Pause playing
    // curl -H Authorization:<token> localhost:8080/pause
    protected.get("pause") { req -> Response in
        audioPlayer.pause()
        return Response(status: .ok)
    }

    // Resume playing
    // curl -H Authorization:<token> localhost:8080/resume
    protected.get("resume") { req -> Response in
        audioPlayer.resume()
        return Response(status: .ok)
    }

    // Shuffle the order of the playing queue
    // curl -H Authorization:<token> localhost:8080/shuffle
    protected.get("shuffle") { req -> Response in
    ...
    // curl -H Authorization:<token> localhost:8080/resume
    protected.get("queue") { req -> PlayingQueue in
        return listQueue()
    }
    ...
    // curl -H Authorization:<token> localhost:8080/levels
    protected.get("levels") { req -> AudioLevels in
    ...
    // an idle connection is silent. See StreamSession. Registered on `protected`,
    // so AuthMiddleware gates the HTTP upgrade request itself: an unauthorized
    // request gets a normal 401 response and the socket never upgrades (verified
    // empirically — see the F70 commit message). The in-closure check below is a
    // second, redundant belt-and-suspenders guard (same rule: loopback or token)
    // kept from before the middleware existed, using the non-throwing form since
    // by this point we're already committed to the WebSocket protocol.
    protected.webSocket("stream") { req, ws in
        let authControl = AuthController(pairing: pairingService, trackFinder: trackFinder)
    ...
    // curl -H Authorization:<token> localhost:8080/playuntil/1750000000
    // NOTE (F32 scoping): despite being named in the audit's callout list, this
    ...
    protected.get("playuntil", ":timestamp") { req -> PlayingQueue in
...
func volumeRoutes(_ protected: any RoutesBuilder) throws {

    // list every stored volume adjustment
    // curl -H Authorization:<token> localhost:8080/volume
    ...
    protected.get("volume") { req async throws -> [VolumeAdjustment] in
    ...
    // curl -H Authorization:<token> localhost:8080/volume/for/8ba165d9fe8f1050687dfa0f34ab42df6a29e72c
    // F32: effectiveGainDecibels does a real synchronous SQLite read; offloaded.
    protected.get("volume", "for", ":sha1") { req async throws -> VolumeAdjustment in
    ...
    // curl -H Authorization:<token> localhost:8080/volume/live/6.5
    // NOTE (F32 scoping): only touches in-RAM audioPlayer state, no DB — left sync.
    protected.get("volume", "live", ":decibels") { req -> Response in
    ...
    // curl -H Authorization:<token> -H 'content-type: application/json' -d '{"scope":"track","sha1":"…","decibels":6}' localhost:8080/volume
    // F32: setVolumeAdjustment is a real synchronous SQLite write; offloaded.
    protected.post("volume") { req async throws -> Response in
    ...
    // curl -H Authorization:<token> -H 'content-type: application/json' -d '{"scope":"track","sha1":"…","decibels":0}' localhost:8080/volume/clear
    // F32: clearVolumeAdjustment is a real synchronous SQLite write; offloaded.
    protected.post("volume", "clear") { req async throws -> Response in
        let adj = try req.content.decode(VolumeAdjustment.self)
        try await offload {
    ...
    // curl -H Authorization:<token> localhost:8080/volume/master
    // F32: masterGainDecibels reads through the database's `meta` table
    // synchronously; offloaded.
    protected.get("volume", "master") { req async throws -> MasterVolume in
    ...
    // curl -H Authorization:<token> -H 'content-type: application/json' -d '{"decibels":-6}' localhost:8080/volume/master
    // F32: setMasterGainDecibels is a real synchronous SQLite write; offloaded.
    protected.post("volume", "master") { req async throws -> Response in
...
func routes(_ app: Application) throws {
    ...
    // Every route that used to open with `AuthController(...).headerAuth`/`track`/
    // `trackFromPath` (the ~30 sites in this file, ~3 more in PairingController.swift)
    // now lives on this group instead, gated once by AuthMiddleware rather than
    // per-handler. AuthMiddleware accepts EITHER the `Authorization` header OR an
    // `auth` path parameter, so /stream/:auth/:sha1 (whose token travels in the
    // path) is registered here too rather than needing a separate ungated route.
    let protected = app.grouped(AuthMiddleware(pairing: pairingService))

    try trackServingRoutes(protected)
    try historyRoutes(protected)
    try playerRoutes(protected)
    try volumeRoutes(protected)
    try pairingRoutes(app, protected: protected)
}
